...
def compute_portvals(orders_file = "./orders/orders.csv", start_val = 1000000, commission=0.0, impact=0.0):
    ...
    portvals_lst = []
    price = get_data(list(data['Symbol'].unique()),pd.date_range(start_date, end_date))
    date_trade = price.index
    current_status = pd.DataFrame({'symbol':'','shares':''}, index = [])

    order_date = list(data['Date'])
    for i in date_trade:
        ...
        if i in order_date:
            ...
            for order_index in range(len(order)):
                ...
                date = daily_order[0]
                symbol = daily_order[1]
                action = daily_order[2]
                shares = daily_order[3]
            ...
            # adj_price = np.array(get_data(current_symbol, pd.date_range(i, i))[current_symbol].iloc[0])
            # print current_symbol, i, adj_price
            portvals = (adj_price * np.array(current_status['shares'])).sum()
            final_portval = start_val + portvals
            portvals_lst.append(final_portval)
