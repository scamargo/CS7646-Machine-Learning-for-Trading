...
class ManualStrategy():
    ...
    def val_plot(self, num,df, benchmark, df_trade, title):
        ...
        xcoords = list(zip(list(df_trade.Date), list(df_trade.trade)))
        for xc in xcoords:
            if xc[1] < 0:
...
if __name__ == "__main__":
    ...
    df_both_in = tos.testPolicy_Qstick(symbol,sd=sd_insample, ed=ed_insample, sv = 100000)
    ...
    df_both_out = tos.testPolicy_Qstick(symbol,sd=sd, ed=ed, sv = 100000)



    port_val_both_in = marketsim.compute_portvals(data = df_both_in,sd=sd_insample, ed=ed_insample, symbol=symbol, commission=9.95, impact=0)[0]
    port_val_both_out = marketsim.compute_portvals(data = df_both_out,sd=sd, ed=ed, symbol=symbol, commission=9.95, impact=0)[0]

    port_val_Qstick = marketsim.compute_portvals(data = df_Qstick,sd=sd_insample, ed=ed_insample, symbol=symbol, commission=9.95, impact=0)[0]
    port_val_bands = marketsim.compute_portvals(data = df_bands,sd=sd_insample, ed=ed_insample, symbol=symbol, commission=9.95, impact=0)[0]
    ...
    tos.val_plot(0,port_val_Qstick_normal,benchmark_normal, df_Qstick,'Qstick')
    tos.val_plot(1,port_val_bands_normal,benchmark_normal, df_bands,'Bollinger Bands(threshold = 1)')
